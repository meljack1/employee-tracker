...
class Queries {
    ...
    viewBudget(department, func) {
        this.db.query(`SELECT SUM(role.salary) AS "Total Utilised Budget" FROM employee JOIN role ON employee.role_id = role.id WHERE role.department_id = ${department};`, function (err, result) {
            console.log(`\n`);
            console.table(result);
            func();
        }); 
